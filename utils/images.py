...
async def generate_profile_card(
    ...
    background.rectangle(
        (20, 20), width=860, height=310, color=(0, 0, 0, panel_opacity), radius=20
    )
    ...
    background.text(
        (start_x, 95), f"РАНГ: {safe_rank}", font=font_rank, color=theme_color
    )
    ...
    if streak > 0:
        ...
        background.text(
            (840, 95), streak_text, font=font_rank, color="#FF5733", align="right"
        )
        try:
    ...
    background.text(
        (start_x, 125), f"VibeКоины: {vibecoins:,}", font=font_text, color="#F1C40F"
    )
    v_hours = voice_seconds // 3600
    v_mins = (voice_seconds % 3600) // 60
    background.text(
        (start_x + 220, 125),
        f"В голосе: {v_hours}ч {v_mins:02d}м",
        font=font_text,
        color="#A6A1FD",
    )
    ...
    percentage = (
        min(max((xp_in_level / xp_needed) * 100, 0), 100) if xp_needed > 0 else 0
    )
    ...
    if user_achievements:
        ...
        if len(user_achievements) > 8:
            ...
            background.text(
                (x_pos + 10, 240), f"+{extra}", font=font_text, color="#f1c40f"
            )
    else:
        background.text(
            (start_x + 90, 240), "Пока нет трофеев...", font=font_small, color="#555555"
        )
