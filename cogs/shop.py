...
SHOP_ITEMS = {
    "nickname": {
        "name": "🏷️ Погоняло",
        "price": 1000,
        "desc": "Сменить ник любому участнику на 1 час.",
    },
    "fake_status": {
        "name": "🎭 Фейковый статус",
        "price": 500,
        "desc": "Добавляет любую приписку к твоему нику на 1 час.",
    },
    "xp_boost": {
        "name": "⚡ Буст опыта x2",
        "price": 2500,
        "desc": "Удваивает весь получаемый опыт в чате и голосе на 2 часа.",
    },
    "voice_meme": {
        "name": "🔊 Рандомный высер",
        "price": 2000,
        "desc": "Бот будет заходить к тебе в войс и кидать мемные звуки целый час.",
    },
}
...
class GameDuelInviteView(View):
    def __init__(self, bot, challenger_id, target_id, bet, game_type):
        ...
        self.bot, self.challenger_id, self.target_id, self.bet, self.game_type = (
            bot,
            challenger_id,
            target_id,
            bet,
    ...
    async def btn_accept(self, interaction: discord.Interaction, button: Button):
        ...
        if not challenger:
            await interaction.followup.send("❌ Инициатор покинул сервер.")
            return
        ...
        await db.update_user(
            str(self.challenger_id), vibecoins=u1_data["vibecoins"] - self.bet
        )
        await db.update_user(
            str(self.target_id), vibecoins=u2_data["vibecoins"] - self.bet
        )
        ...
        if self.game_type == "bj":
            ...
            msg = await interaction.channel.send(
                content=f"{challenger.mention} 🆚 {interaction.user.mention}",
...
class GameDuelSelectUser(UserSelect):
    def __init__(self, bot, challenger_id, bet, game_type):
        ...
        self.bot, self.challenger_id, self.bet, self.game_type = (
            bot,
            challenger_id,
            bet,
            game_type,
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if target.bot or target.id == self.challenger_id:
            await interaction.response.send_message(
                "❌ Недопустимая цель!", ephemeral=True
            )
            return
        ...
        view = GameDuelInviteView(
            self.bot, self.challenger_id, target.id, self.bet, self.game_type
        )
        await interaction.channel.send(
            content=f"⚔️ <@{self.challenger_id}> вызывает {target.mention} на дуэль!",
            view=view,
        )
...
class ShopView(View):
    ...
    async def _check_existing_thread(
        ...
        for thread in interaction.channel.threads:
            if (
                not thread.archived
                and prefix in thread.name.lower()
                and search_name in thread.name.lower()
            ):
                return thread
        return None
    ...
    async def buy_nickname(self, interaction, button):
        await interaction.response.send_message(
            "🏷️ Выбери цель:", view=NicknameSelectView(), ephemeral=True
        )

    @discord.ui.button(
        label="🎭 Статус (500)",
        style=discord.ButtonStyle.secondary,
        custom_id="shop_status",
        row=0,
    )
    ...
    async def buy_xp(self, interaction, button):
        ...
        if is_active or u_data.get("xp_boost_remaining", 0) > 0:
            await interaction.followup.send(
                "❌ У тебя уже есть активный или приостановленный буст! Дождись его окончания, чтобы купить новый.",
                ephemeral=True,
            )
            return
        ...
        await interaction.followup.send(
            "⚡ Буст x2 на 2 часа куплен! Он будет работать, когда ты общаешься или сидишь в войсе с кем-то.",
            ephemeral=True,
        )
        interaction.client.dispatch(
            ...
            interaction.user,
            "xp_boost",
            u_data["vibecoins"] - 2500,
            u_data.get("nick_changes", 0),
        )

    @discord.ui.button(
        label="🔊 Мемы (2k)",
        style=discord.ButtonStyle.secondary,
        custom_id="shop_voice_meme",
        row=0,
    )
    async def buy_meme(self, interaction, button):
        ...
        await db.update_user(
            str(interaction.user.id),
            vibecoins=u_data["vibecoins"] - 2000,
    ...
    async def go_casino(self, interaction, button):
        ...
        if existing:
            await interaction.response.send_message(
                f"❌ Есть открытый стол: {existing.mention}", ephemeral=True
            )
            return
        await interaction.response.defer(ephemeral=True)
        thread = await interaction.channel.create_thread(
            name=f"🎰┃казино-{interaction.user.name[:10]}",
        ...
        await thread.send(
            embed=get_casino_embed(interaction.user.display_name), view=CasinoView()
        )
        await interaction.followup.send(
            f"✅ Ветка создана: {thread.mention}", ephemeral=True
        )
    ...
    async def go_cases(self, interaction, button):
        ...
        if existing:
            await interaction.response.send_message(
                f"❌ Есть открытая комната: {existing.mention}", ephemeral=True
            )
            return
    ...
    async def go_duels(self, interaction, button):
        ...
        if existing:
            await interaction.response.send_message(
                f"❌ Есть активная комната: {existing.mention}", ephemeral=True
            )
            return
    ...
    async def go_bj(self, interaction, button):
        ...
        if existing:
            await interaction.response.send_message(
                f"❌ Твой стол уже здесь: {existing.mention}", ephemeral=True
            )
            return
        await interaction.response.defer(ephemeral=True)
        thread = await interaction.channel.create_thread(
            name=f"🃏┃блэкджек-{interaction.user.name[:10]}",
            type=discord.ChannelType.private_thread,
        )
        await thread.add_user(interaction.user)
        if interaction.guild.owner:
        ...
        await thread.send(
            embed=discord.Embed(
                title="🃏 БЛЭКДЖЕК", description=desc, color=COLOR_MAIN
            ),
            view=BlackjackRoomView(interaction.client),
        )
    ...
    async def go_quiz(self, interaction, button):
        ...
        if existing:
            await interaction.response.send_message(
                f"❌ Ты не закончил викторину: {existing.mention}", ephemeral=True
            )
            return
        ...
        try:
            ...
            await thread.send(
                embed=discord.Embed(
                    title="💡 ВИКТОРИНА", description=desc, color=COLOR_MAIN
                ),
                view=QuizRoomView(interaction.client),
            )
            await interaction.followup.send(
                f"✅ Готово: {thread.mention}", ephemeral=True
            )
            self._start_cleanup_task(thread)
            ...
            await interaction.followup.send(
                f"❌ Ошибка при создании ветки: {e}", ephemeral=True
            )
...
class Shop(commands.Cog):
    ...
    async def setup_shop(self, ctx):
        embed = discord.Embed(
            ...
            description="Трать свои **VibeКоины** на бонусы и игры!",
            color=COLOR_MAIN,
        )
        ...
        embed.add_field(
            name="──────────────", value="**🎮 ИГРОВЫЕ КОМНАТЫ**", inline=False
        )
        embed.set_image(
            url="https://media.giphy.com/media/xUPGGw7jzcqeMw5dI8/giphy.gif"
        )
...
class NicknameModal(Modal):
    def __init__(self, target):
        ...
        self.target, self.nick_input = target, TextInput(
...
class FakeStatusModal(Modal):
    def __init__(self):
        ...
        self.status_input = TextInput(
            label="Статус", placeholder="[BOSS]", max_length=15
        )
