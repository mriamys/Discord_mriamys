...
class ShopView(View):
    ...
    async def _check_existing_thread(self, interaction: discord.Interaction, prefix: str):
        """Проверяет, есть ли у пользователя уже открытая ветка с таким префиксом."""
    ...
    @discord.ui.button(label="🎰 Казино", style=discord.ButtonStyle.success, custom_id="shop_casino_v2", row=2)
    ...
    @discord.ui.button(label="📦 Кейсы", style=discord.ButtonStyle.success, custom_id="shop_cases_v2", row=2)
    ...
    @discord.ui.button(label="⚔️ Дуэли", style=discord.ButtonStyle.success, custom_id="shop_duels_v2", row=2)
    ...
    @discord.ui.button(label="🃏 Блэкджек", style=discord.ButtonStyle.success, custom_id="shop_bj_v2", row=3)
    ...
    async def go_quiz(self, interaction: discord.Interaction, button: Button):
        ...
        desc = "**ПРАВИЛА:**\n🔹 Соло: вопрос из интернета.\n🔹 Дуэль: кто первый правильно нажал - забирает банк!"
    ...
    # ... (Остальные кнопки покупки остаются без изменений)
    ...
    async def buy_status(self, interaction: discord.Interaction, button: Button):
        from cogs.shop import FakeStatusModal
...
# Заглушки для предотвращения ImportError
...
class FakeStatusModal(Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if user_data.get('vibecoins', 0) < 500:
            await interaction.followup.send("❌ Мало коинов!", ephemeral=True); return
        try:
            await interaction.user.edit(nick=f"{interaction.user.display_name} | {self.status_input.value}"[:32])
