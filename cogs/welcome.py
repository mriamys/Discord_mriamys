...
class Welcome(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        try:
            ...
            roles_channel = discord.utils.get(
                member.guild.text_channels, name="🎭┃выдача-ролей"
            )
            roles_mention = (
                roles_channel.mention if roles_channel else "канале с выдачей ролей"
            )
            ...
            if image_bytes:
                ...
                logging.info(
                    f"Welcome message sent to #{channel.name} for {member.name}"
                )
