    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(
                "❌ Введите корректное число (минимум 10)!", ephemeral=True
            )
            return
        ...
        if self.game_type == "solo":
            ...
            await interaction.response.send_message(
                f"🃏 Вызываем на дуэль! Ставка: **{bet} 🪙**",
                view=GameDuelSelectView(self.bot, interaction.user.id, bet, "bj"),
                ephemeral=True,
            )
...
class BlackjackBetView(View):
    ...
    async def bet_500(self, interaction, button):
    ...
    async def bet_custom(self, interaction, button):
        await interaction.response.send_modal(
            BlackjackBetModal(self.bot, self.game_type)
        )
...
class BlackjackView(View):
    ...
    async def create_embed(self):
        ...
        if self.game.status == "playing":
            ...
            embed.add_field(
                name="🤖 ДИЛЕР", value=f"Карты: {d_cards}\nСчет: **?**", inline=False
            )
            embed.description = (
                f"💰 Ставка: **{self.bet} 🪙** | Баланс: **{balance:,} 🪙**"
            )
        else:
            ...
            embed.add_field(
                name="🤖 ДИЛЕР",
                value=f"Карты: {format_hand(self.game.dealer_hand)}\nСчет: **{d_score_display}**",
                inline=False,
            )
    ...
    async def hit(self, interaction: discord.Interaction, button: Button):
        if interaction.user.id != self.member.id or self.processing:
            return
    ...
    async def end_game(self, interaction):
        ...
        new_bj_wins = user_data.get("bj_wins", 0) + (
            1 if self.game.status == "player_win" else 0
        )
        await db.update_user(
            str(self.member.id), vibecoins=new_balance, bj_wins=new_bj_wins
        )
        ...
        await interaction.channel.send(
            embed=discord.Embed(
                title="🃏 БЛЭКДЖЕК", description="Хотите сыграть еще?", color=0x2B2D31
            ),
            view=BlackjackRoomView(self.bot),
        )
...
class BlackjackDuelView(View):
    def __init__(self, bot, p1, p2, bet):
        ...
        self.players = {
            p.id: {"member": p, "hand": [], "status": "playing"} for p in [p1, p2]
        }
        ...
        for pid, data in self.players.items():
            ...
            if self.game_over:
                ...
                if data["status"] != "playing":
                    ...
                    first_card = (
                        f"`{RANKS.get(data['hand'][0][0])} {data['hand'][0][1]}`"
                    )
    ...
    async def resolve_winner(self, interaction):
        ...
        if winner_id:
            ...
            winner, loser = (
                self.players[winner_id]["member"],
                self.players[loser_id]["member"],
            )
            ...
            await db.update_user(
                str(winner_id),
                vibecoins=w_data["vibecoins"] + (self.bet * 2),
                bj_wins=new_wins,
            )
        ...
        cont_view = BlackjackDuelContinueView(
            self.bot,
            self.players[p1_id]["member"],
            self.players[p2_id]["member"],
            self.bet,
        )
        await interaction.channel.send(
...
class BlackjackDuelContinueView(View):
    ...
    @discord.ui.button(
        label="🔄 Реванш (Та же ставка)", style=discord.ButtonStyle.primary
    )
    async def rematch(self, interaction: discord.Interaction, button: Button):
        if interaction.user.id not in [self.p1.id, self.p2.id] or self.started:
            return
        self.started = True
    ...
    async def change_bet(self, interaction: discord.Interaction, button: Button):
        ...
        await interaction.response.send_message(
            ...
            view=BlackjackBetView(self.bot, "duel"),
            ephemeral=True,
        )
    ...
    async def exit(self, interaction: discord.Interaction, button: Button):
        if interaction.user.id not in [self.p1.id, self.p2.id]:
            return
        await interaction.response.send_message(
            f"🚪 {interaction.user.display_name} покинул игру."
        )
...
class BlackjackRoomView(View):
    ...
    @discord.ui.button(
        ...
        custom_id="bj_solo_btn",
    )
    async def solo(self, interaction: discord.Interaction, button: Button):
        await interaction.response.send_message(
            "💰 Выберите размер вашей ставки:",
            view=BlackjackBetView(self.bot, "solo"),
            ephemeral=True,
        )

    @discord.ui.button(
        label="⚔️ Вызвать на Дуэль",
        style=discord.ButtonStyle.success,
        custom_id="bj_duel_btn",
    )
    ...
    @discord.ui.button(
        ...
        custom_id="bj_close_btn",
    )
    async def exit(self, interaction: discord.Interaction, button: Button):
        await interaction.response.send_message("🚪 Уходим из казино...")
        await asyncio.sleep(2)
        try:
            await interaction.channel.delete()
        except:
