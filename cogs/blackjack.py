    ...
    async def create_embed(self):
        ...
        for pid, data in self.players.items():
            ...
            if self.game_over:
                ...
                if self.turn == pid:
                    status_text = "🎯 **ВАШ ХОД**"
                    # Активный игрок видит свои карты
                    cards_text = format_hand(data["hand"])
                    score_text = f"Счет: **{self.get_score(data['hand'])}**"
                else:
                    status_text = "⌛ ОЖИДАНИЕ..."
                    # Оппонент видит только одну карту
