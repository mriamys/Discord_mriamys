...
class LevelUpView(discord.ui.View):
    ...
    async def show_profile(
        ...
        if economy_cog and str(interaction.user.id) in economy_cog.voice_sessions:
            current_session = int(
                time.time() - economy_cog.voice_sessions[str(interaction.user.id)]
            )
        ...
        async with db.pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    "SELECT bg_color FROM profile_settings WHERE user_id = %s",
                    (str(interaction.user.id),),
                )
                profile_settings = await cur.fetchone()

        bg_color = (
            profile_settings.get("bg_color", "#2b2d31")
            if profile_settings
            else "#2b2d31"
        )
        ...
        image_bytes = await generate_profile_card(
            interaction.user,
            level,
            xp,
            vibecoins,
    ...
    @discord.ui.button(
        ...
        custom_id="levelup_shop",
    )
    async def go_shop(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        shop_channel = discord.utils.get(
            interaction.guild.text_channels, name="🛒┃магазин"
        )
        if not shop_channel:
            shop_channel = discord.utils.get(
                interaction.guild.text_channels, name="магазин"
    ...
    async def show_trophies(
        ...
        if not user_achievements:
            await interaction.response.send_message(
                f"У {self.target_member.display_name} пока нет трофеев.", ephemeral=True
            )
            return
        ...
        sorted_ach = sorted(
            ...
            key=lambda x: RARITY_ORDER.get(
                ACHIEVEMENTS.get(x, {}).get("rarity", "common"), 0
            ),
            reverse=True,
        )

        embed = discord.Embed(
            title=f"🏆 Трофеи: {self.target_member.display_name}", color=0xF1C40F
        )
    ...
    @discord.ui.select(
        ...
        options=[
            discord.SelectOption(
                label="🏆 Уровень (XP)",
                value="level",
                description="Самые прокачанные пользователи",
            ),
            discord.SelectOption(
                label="💰 VibeКоины",
                value="coins",
            ...
            discord.SelectOption(
                label="🎙️ Голосовой актив",
                value="voice",
                description="Больше всего времени в войсах",
            ),
            discord.SelectOption(
                label="🔥 Стрики",
                value="streak",
                description="Самые длинные серии общения",
            ),
        ],
    )
...
class Leveling(commands.Cog):
    ...
    @commands.hybrid_command(
        name="top", aliases=["топ"], description="Посмотреть списки лидеров сервера"
    )
        ...
        if role and role not in member.roles:
            ...
            if new_level > old_level:
                ...
                rank_channel = discord.utils.get(
                    member.guild.text_channels, name="📜┃ранг"
                )
    ...
    @commands.hybrid_command(
        name="profile",
        aliases=["ранг", "rank"],
        description="Показать твою или чужую карточку профиля",
    )
    async def profile(self, ctx, member: discord.Member = None):
        ...
        if is_wrong_channel:
            if ctx.interaction:
                await ctx.interaction.response.send_message(
                    f"Перейди в канал {rank_channel.mention}, твой профиль сгенерирован там!",
                    ephemeral=True,
                )
            else:
                try:
                    ...
                    pass
                msg = await ctx.send(
                    f"{ctx.author.mention}, твой профиль отправлен в {rank_channel.mention}!"
                )
                try:
                    import asyncio
        ...
        if is_wrong_channel:
            await rank_channel.send(
                content=f"{ctx.author.mention}, твой профиль:", file=file, view=view
            )
        else:
    ...
    @commands.hybrid_command(
        name="stat",
        aliases=["стат", "статистика", "stats"],
        description="Показать детальную текстовую статистику пользователя",
    )
    async def stat(self, ctx, member: discord.Member = None):
        ...
        embed = discord.Embed(
            title=f"📊 Статистика: {member.display_name}", color=0x2B2D31
        )
        ...
        embed.add_field(
            name="🏆 Уровень / XP", value=f"Ур. {level} ({xp} XP)", inline=True
        )
        embed.add_field(name="💰 VibeКоины", value=f"{vibecoins}", inline=True)
        embed.add_field(
            name="🌟 Ачивки", value=f"{len(achievements)} / {total_ach}", inline=True
        )
        ...
        if v_until and v_until > now and v_count < 10:
            ...
            mins_left = int(diff.total_seconds() // 60)
            bonuses.append(
                f"🔊 **Рандом высер**: {10 - v_count}/10 раз (ещё {mins_left} мин.)"
            )
