...
class GameSelect(discord.ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(
                label=game, emoji=emoji, description=f"Роль игрока {game}"
            )
    ...
    async def callback(self, interaction: discord.Interaction):
            ...
            if not category:
                ...
                try:
                    ...
                    await guild.create_text_channel(
                        f"💬┃{game.lower()}-chat", category=category
                    )
                    await guild.create_voice_channel(
                        f"🔊┃{game.upper()}", category=category
                    )
                ...
                try:
                    await category.set_permissions(
                        role, read_messages=True, connect=True
                    )
                    await category.set_permissions(
        ...
        await interaction.followup.send(
            "Ваши игровые роли успешно обновлены! (Были добавлены нужные чаты, если их не было)",
            ephemeral=True,
        )
...
class DevSelect(discord.ui.Select):
    def __init__(self):
        ...
        for dev, emoji in DEV_OPTIONS.items():
            ...
            options.append(
                discord.SelectOption(label=dev, emoji=emoji, description=desc)
            )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for dev in DEV_OPTIONS.keys():
            if "кодер" in dev.lower() or dev == "Программист":
                role_name = dev
            else:
        ...
        for dev in selected_devs:
            if "кодер" in dev.lower() or dev == "Программист":
                role_name = dev
                old_role_name = f"{dev} Coder" if dev != "Программист" else None
            else:
                role_name = f"{dev} Coder"
                old_role_name = None
            ...
            if not role and old_role_name:
                role = discord.utils.get(guild.roles, name=old_role_name)
                if role:
                    try:
                        await role.edit(name=role_name)
                        print(f"Роль переименована: {old_role_name} -> {role_name}")
                    except discord.Forbidden:
                        print(f"Нет прав переименовать роль {old_role_name}")
        ...
        # Если была выбрана хотя бы одна галочка, даем базовую роль "Программист"
        if selected_devs:
            if not base_prog_role:
                base_prog_role = await guild.create_role(
                    name="Программист",
                    mentionable=True,
                    hoist=True,
                    color=discord.Color.from_rgb(46, 204, 113),
        ...
        if category:
            try:
                if selected_devs and base_prog_role:
                    await category.set_permissions(
                        base_prog_role, read_messages=True, connect=True
                    )
                for dev in selected_devs:
                    if "кодер" in dev.lower() or dev == "Программист":
                        role_name = dev
            ...
            # Создаем текстовые чаты
            for dev in selected_devs:
                if dev == "Программист":
                    continue

                if "кодер" in dev.lower():
                    role_name = dev
                else:
                    role_name = f"{dev} Coder"
                ...
                existing_channel = discord.utils.get(
                    guild.text_channels, name=chan_name
                )
                ...
                if not existing_channel:
                    try:
                        await guild.create_text_channel(
                            chan_name, category=category, overwrites=chan_overwrites
                        )
                    except Exception as e:
                        print(e)
        ...
        await interaction.followup.send(
            "Ваши роли программиста обновлены!", ephemeral=True
        )
...
class RolesCog(commands.Cog):
    ...
    async def setup_roles(self, ctx):
        ...
        game_embed.set_image(
            url="https://i.pinimg.com/originals/a6/90/13/a690132b904eb12cccf80e927c7cfbc2.gif"
        )  # Красивая игровая гифка
        ...
        dev_embed.set_image(
            url="https://i.pinimg.com/originals/f3/9d/23/f39d2319bf30c4f8280695ee91b945ba.gif"
        )  # Хакерская/Кодерская гифка
