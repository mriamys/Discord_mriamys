...
class DynamicRooms(commands.Cog):
    ...
    async def setup_dynamic_voice(self, ctx):
        ...
        trigger_channel = await guild.create_voice_channel(
            name="➕ создать комнату", category=category
        )
        embed = discord.Embed(
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if after.channel is not None and any(
            t in after.channel.name.lower() for t in self.trigger_channel_names
        ):
            ...
            try:
                ...
                new_channel = await guild.create_voice_channel(
                    name=f"🎮┃{member.display_name}", category=category, user_limit=0
                )
