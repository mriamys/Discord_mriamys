...
class CaseView(View):
    ...
    async def _handle_case(
        ...
        if str(interaction.user.id) != owner_id:
            await interaction.response.send_message(
                "❌ Это не твой кейс! Открой свой через команду.", ephemeral=True
            )
            return
        ...
        if balance < price:
            await interaction.response.send_message(
                f"❌ Не хватает **VibeКоинов** для {case_name}! У тебя {balance}/{price} 🪙",
                ephemeral=True,
            )
            return
        ...
        embed = discord.Embed(
            title=f"{emoji} Открытие {case_name}...",
            description="[ 🎰 ] КРУТИМ РУЛЕТКУ [ 🎰 ]",
        ...
        if is_case_channel:
            ...
            await interaction.channel.send(
                content=interaction.user.mention, embed=menu_embed, view=CaseView(uid)
            )
    ...
    async def btn_jade(self, interaction: discord.Interaction, button: Button):
        await self._handle_case(
            interaction, 8000, 2000, 30000, "Нефритового Кейса", "🔮"
        )
    ...
    @discord.ui.button(
        ...
        emoji="💎",
        row=1,
        custom_id="case_diamond",
    )
    async def btn_diamond(self, interaction: discord.Interaction, button: Button):
        await self._handle_case(
            interaction, 50000, 10000, 250000, "Бриллиантового Кейса", "💎"
        )
    ...
    async def btn_ruby(self, interaction: discord.Interaction, button: Button):
        await self._handle_case(
            interaction, 250000, 50000, 1500000, "Рубинового Кейса", "🔴"
        )
    ...
    @discord.ui.button(
        ...
        emoji="🧿",
        row=2,
        custom_id="case_sapphire",
    )
    async def btn_sapphire(self, interaction: discord.Interaction, button: Button):
        await self._handle_case(
            interaction, 500000, 100000, 3000000, "Сапфирового Кейса", "🧿"
        )
    ...
    @discord.ui.button(
        ...
        emoji="🌌",
        row=2,
        custom_id="case_ultimate",
    )
    async def btn_ultimate(self, interaction: discord.Interaction, button: Button):
        await self._handle_case(
            interaction, 1000000, 250000, 7000000, "Vibe Ultimate Кейса", "🌌"
        )
...
class Cases(commands.Cog):
    ...
    async def on_create_vibe_case_room(self, interaction: discord.Interaction):
        ...
        await thread.send(
            ...
            embed=embed,
            view=CaseView(str(interaction.user.id)),
        )
