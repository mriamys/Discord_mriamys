...
class MusicControlView(View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if not interaction.user.voice or not interaction.user.voice.channel:
            await interaction.response.send_message(
                "❌ Зайдите в голосовой канал!", ephemeral=True
            )
            return False
        ...
        if vc and interaction.user.voice.channel != vc.channel:
            ...
            if len(vc.channel.members) == 1:
                ...
                return True
            await interaction.response.send_message(
                f"❌ Нужно быть в одном канале с ботом!", ephemeral=True
            )
            return False
        return True
    ...
    async def repeat_btn(self, interaction: discord.Interaction, button: Button):
        ...
        await interaction.response.edit_message(
            embed=self.cog.create_embed(state, state.current_track), view=self
        )
    ...
    async def stop(self, interaction: discord.Interaction, button: Button):
        ...
        if vc:
            ...
            await interaction.response.edit_message(
                content="⏹️ Плеер остановлен.", embed=None, view=None
            )
...
class Music(commands.Cog):
    ...
    def create_embed(self, state, track_data):
        if not track_data:
            return discord.Embed(
                title="🎵 Плеер", description="Ничего не играет", color=COLOR_SUCCESS
            )
        ...
        requester = (
            f"<@{track_data['user_id']}>" if "user_id" in track_data else "Система"
        )

        embed = discord.Embed(
            title="🎵 Сейчас играет",
            description=f"**[{title}]({track_data.get('url')})**",
            color=COLOR_SUCCESS,
        )
        if thumbnail:
        ...
        embed.add_field(
            name="🔄 Повтор", value="✅ Вкл" if state.repeat else "❌ Выкл", inline=True
        )
    ...
    async def play(self, ctx, *, search: str):
        ...
        if "spotify.com/track" in search:
            ...
            if s_title:
                ...
                return await ctx.send("❌ Не удалось получить информацию из Spotify.")
        elif not search.startswith("http"):
            search = f"ytsearch:{search}"

        active_users = set(t["user_id"] for t in state.queue)
        if state.current_track:
            active_users.add(state.current_track["user_id"])
        is_alone = len(active_users) <= 1 and (
            not active_users or ctx.author.id in active_users
        )
        playlist_limit = 50 if is_alone else 5

        try:
            data = await self.bot.loop.run_in_executor(
                None, lambda: ytdl.extract_info(search, download=False, process=False)
            )
            if data is None:
            ...
            if not tracks_to_add:
                return await ctx.send("❌ Не удалось найти треки.")
            ...
            for track in tracks_to_add:
                if (
                    ...
                    and not state.current_track
                ):
                    state.current_track = track
                    player = await YTDLSource.from_url(
                        track["url"], loop=self.bot.loop, stream=True
                    )
                    if player:
            ...
            if len(tracks_to_add) > 1:
                await ctx.send(
                    f"✅ Добавлено **{len(tracks_to_add)}** треков. (Лимит: {playlist_limit})",
                    delete_after=10,
                )
            elif not started_playing:
                await ctx.send(
                    f"➕ Добавлено в очередь: **{tracks_to_add[0]['title']}**",
                    delete_after=10,
                )
            else:
                await ctx.send(
                    f"▶️ Начинаю играть: **{tracks_to_add[0]['title']}**",
                    delete_after=10,
                )
    ...
    async def async_play_next(self, ctx):
        ...
        if next_track:
            ...
            if state.controls_msg:
                try:
                    await state.controls_msg.edit(
                        content="⏹️ Очередь пуста.", embed=None, view=None
                    )
                except:
                    pass
    ...
    async def play_error(self, ctx, error):
        ...
        try:
            if isinstance(original, asyncio.TimeoutError):
                ...
                msg = f"❌ Произошла ошибка: `{type(original).__name__}`"
            # Try to reply; if already responded just send a follow-up
            if ctx.interaction and not ctx.interaction.response.is_done():
                await ctx.interaction.response.send_message(msg, ephemeral=True)
            else:
                await ctx.send(msg, ephemeral=True)
        except Exception as e:
            logging.error(f"play_error: failed to send error message: {e}")
    ...
    async def on_voice_state_update(self, member, before, after):
        if member.id == self.bot.user.id and after.channel is None:
            ...
            if state.controls_msg:
                try:
                    await state.controls_msg.edit(
                        content="⏹️ Плеер остановлен (бот отключен).",
                        embed=None,
                        view=None,
                    )
                except:
    ...
    @commands.hybrid_command(
        name="queue", description="Показать очередь воспроизведения"
    )
    async def queue(self, ctx):
        ...
        if state.current_track:
            embed.add_field(
                ...
                value=state.current_track.get("title", "Без названия"),
                inline=False,
            )
        ...
        if state.queue:
            ...
            embed.add_field(
                name="📋 Будущие треки", value="Очередь пуста", inline=False
            )

        await ctx.send(embed=embed)

    @commands.hybrid_command(
        name="stop", description="Остановить плеер и очистить очередь"
    )
